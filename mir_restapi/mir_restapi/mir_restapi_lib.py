...
class HttpConnection():
    ...
    def put(self, path, body=None):
        ...
        except (BrokenPipeError, RemoteDisconnected, ConnectionResetError, socket.timeout) as e:
            print(f"[WARN] Connection failed ({e}), attempting to reconnect...")
            self._reconnect()
            try:
                self.connection.request("PUT", self.api_prefix + path, body=body, headers=self.http_headers)
                response = self.connection.getresponse()
                ...
                print(f"[ERROR] Failed after reconnect: {e}")
                return None
        except Exception as e:
    ...
    def _reconnect(self):
        try:
            ...
            pass
        self.connection = http.client.HTTPConnection(self.host, self.port, timeout=3)
        print("[INFO] HTTP connection re-established.")
    '''
    ...
    def delete(self, path):
        self.connection.request("DELETE", self.api_prefix+path, headers=self.http_headers)
        resp = self.connection.getresponse()
        if resp.status < 200 or resp.status >= 300:
...
class MirRestAPI():
    ...
    def set_state_id(self, stateId):
        # 3 = Ready, 4 = Pause, 11 = Manualcontrol
    ...
    def sync_time(self):
        ...
        try:
            # MiR expects ISO8601 datetime string in a JSON object like {'datetime': '...'}
            result = self.http.put("/status", json.dumps({'datetime': dT}))
            response += str(result)
            return response
        except Exception as e:
            if "timed out" in str(e).lower():
                # Likely expected behavior — REST API restarts, causing a timeout
                response += f"Set datetime to {dT}"
                self.logger.warn("MIR 250 restart in progress - awaiting LED blink sequence completion")
